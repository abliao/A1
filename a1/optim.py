...
class Optimizer(OptimizerBase):
    ...
    def clip_grads_and_collect_metrics(
        ...
        def is_ah(group_name: str) -> bool:
            return "ah" in group_name
        ...
        if is_distributed():  # TODO (epwalsh): skip for non-sharded params
            ...
            if per_param_min_metrics:
                ...
                min_tensors = [t.to(device) for t in per_param_min_metrics]
                all_mins = torch.cat(min_tensors)                
            ...
            if per_param_max_metrics:
                ...
                max_tensors = [t.to(device) for t in per_param_max_metrics]
                all_maxs = torch.cat(max_tensors)
            ...
            norm_tensors = [t.to(device) for t in per_param_norm_metrics]
            all_norms = torch.cat(norm_tensors) ** 2.0
            ...
            if per_param_sum_metrics and per_param_numel_metrics:
                ...
                sum_tensors = [t.to(device) for t in per_param_sum_metrics]
                numel_tensors = [t.to(device) for t in per_param_numel_metrics]
                all_sums = torch.cat(sum_tensors)
                all_numels = torch.cat(numel_tensors)
            ...
            if multi_modal:
                ...
                ah_mask = torch.tensor(
                    [float(is_ah(n)) for n in per_param_group_names], device=all_norms.device
                )
                ...
                ah_grad_norm = (all_norms * grad_norm_metric_mask * ah_mask).sum() ** 0.5
                ...
                ah_grad_norm = (
                    torch.cat(
                        [
                            m
                            for m, n, gn in zip(per_param_norm_metrics, per_param_norm_metric_names, per_param_group_names)
                            if is_grad_norm_metric(n) and is_ah(gn)
                        ]
                    )
                    ** 2.0
                ).sum() ** 0.5
        ...
        if multi_modal:
            ...
            all_metrics["ah_grad_norm"] = ah_grad_norm
        # Clip gradients.
        num_grads_clipped = 0
    ...
    def _do_global_fixed_clipping(
        ...
        if multi_modal:
            ...
            elif "ah" in group_name:
                total_grad_norm = all_metrics["ah_grad_norm"]
            else:
...
class MultimodalScheduler(Scheduler):
    ...
    def get_lr(self, initial_lr: float, step: int, max_steps: int, group_name: str) -> float:
        ...
        elif group_name.startswith("ah"):
            return self.ah_scheduler.get_lr(initial_lr, step, max_steps)
        else:
            raise ValueError(f"Unknown group name: {group_name}")
...
def get_multimodal_param_groups(cfg: TrainConfig, model: nn.Module) -> List[Dict[str, Any]]:
    ...
    ah_params = list(model.get_act_head_parameters())
    pp_params = list(model.get_proprio_proj_parameters())
    ...
    wd_exclusions = list(model.get_weight_decay_exclusions()) #判断该参数是否应该被排除在权重衰减之外
    for mn, m in model.named_modules():
        for pn, p in m.named_parameters():
            ...
            is_ah = listinstr(ah_params, fpn, delimiter='.')
            is_pp = listinstr(pp_params, fpn, delimiter='.')
            is_fm_action_related = listinstr(fm_action_related_params, fpn, delimiter='.')
            no_apply_decay = listinstr(wd_exclusions, fpn, delimiter='.')

            assert is_connector or is_vit or is_llm or is_ah or is_pp or is_fm_action_related, f"Parameter {fpn} does not belong to any group!"
            ...
            elif (is_ah or is_pp or is_fm_action_related) and not no_apply_decay: 
                # ah_decay.add(fpn)
                ah_decay.add(fpn)  # LLM includes action head and proprio projector
            elif (is_ah or is_pp or is_fm_action_related) and no_apply_decay:
                # ah_no_decay.add(fpn)
                ah_no_decay.add(fpn)
            else:
    ...
    ah_inter_params = ah_decay & ah_no_decay
    ...
    inter_params = connector_inter_params | vit_inter_params | llm_inter_params | ah_inter_params
    inter_params = inter_params | connector_vit_inter_params | connector_llm_inter_params | vit_llm_inter_params
    union_params = connector_decay | connector_no_decay | vit_decay | vit_no_decay | llm_decay | llm_no_decay | ah_decay | ah_no_decay
    ...
    ah_decay_sorted = sorted(list(ah_decay))
    ah_no_decay_sorted = sorted(list(ah_no_decay))
    param_groups = []
    ...
    if len(ah_decay_sorted) > 0:
        param_groups.append(
            {
                "params": [all_params[pn] for pn in ah_decay_sorted],
                "param_names": ah_decay_sorted,
                "lr": cfg.optimizer.action_head_learning_rate,
                "weight_decay": cfg.optimizer.action_head_weight_decay,
                "betas": cfg.optimizer.action_head_betas,
                "eps": cfg.optimizer.action_head_eps,
                "group_name": "ah_decay",
                **param_group_defaults,
            }
        )
    if len(ah_no_decay_sorted) > 0:
        param_groups.append(
            {
                "params": [all_params[pn] for pn in ah_no_decay_sorted],
                "param_names": ah_no_decay_sorted,
                "lr": cfg.optimizer.action_head_learning_rate,
                "weight_decay": 0.0,
                "betas": cfg.optimizer.action_head_betas,
                "eps": cfg.optimizer.action_head_eps,
                "group_name": "ah_no_decay",
                **param_group_defaults,
            }
        )
    # Validate fields.
...
def build_multimodal_scheduler(cfg: TrainConfig):
    ...
    connector_sched = CosWithWarmup(
        ...
        freeze_steps=int(sched_cfg.connector_freeze_steps),
    )
    vit_sched = CosWithWarmup(
        ...
        t_max=None if sched_cfg.t_max is None else int(sched_cfg.t_max),
        warmup_min_lr=sched_cfg.warmup_min_lr,
        freeze_steps=int(sched_cfg.vit_freeze_steps),
    )
    llm_sched = CosWithWarmup(
        ...
        t_max=None if sched_cfg.t_max is None else int(sched_cfg.t_max),
        warmup_min_lr=sched_cfg.warmup_min_lr,
        freeze_steps=int(sched_cfg.llm_freeze_steps),
    )
    ah_sched = CosWithWarmup(
        grad_clip_warmup_steps=None
        if sched_cfg.grad_clip_warmup_steps is None
        else int(sched_cfg.grad_clip_warmup_steps),
        grad_clip_warmup_factor=sched_cfg.grad_clip_warmup_factor,
        warmup_steps=int(sched_cfg.action_head_t_warmup),
        alpha_f=sched_cfg.alpha_f,
        t_max=None if sched_cfg.t_max is None else int(sched_cfg.t_max),
        warmup_min_lr=sched_cfg.warmup_min_lr,
        freeze_steps=int(sched_cfg.action_head_freeze_steps),
    )
